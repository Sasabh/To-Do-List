...
export default class TaskTracker extends Component {
  ...
  async addTask(task) {
    ...
    const res = await fetch(
      `https://my-json-server.typicode.com/sasabh/JSON-server/tasks`,
      {
        method: "POST",
        headers: {
  ...
  async deleteTask(id) {
    await fetch(
      `https://my-json-server.typicode.com/sasabh/JSON-server/tasks/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  async toggleReminder(id) {
    ...
    this.setState((state, props) => ({
      tasks: state.tasks.map((task) =>
        task.id === id ? { ...task, doRemind: data.doRemind } : task
      ),
    }));
  }
      ...
      <Fragment>
        <div style={this.boxStyle}>
          <h1 style={{ margin: "1rem 3rem" }}>
            Task Tracker
            <CustomButton
              text={!this.state.showAddTask ? "Add Task" : "Close"}
              color={!this.state.showAddTask ? "#28A745" : "red"}
              customStyle={this.btnStyle}
              onBtnClick={() => this.toggleAdd()}
            />
          </h1>
          <hr style={{ backgroundColor: "black" }} />
          {this.state.showAddTask && (
            <AddTask onAdd={(task) => this.addTask(task)}></AddTask>
          )}
          {this.state.showAddTask && (
            <hr style={{ backgroundColor: "black" }} />
          )}
          {this.state.tasks.length > 0 ? (
            <Tasks
              ...
              onToggle={(id) => this.toggleReminder(id)}
            />
          ) : (
